...
#define DEFAULT_SERVER_IP   0xC0A80164
...
void start() {
    ...
    if (OSEffectiveToPhysical((void *)0xa0000000) != (void *)0x31000000) {
        ...
        unsigned int len = sizeof(wiiurl532_text_bin);
        unsigned char *loc = (unsigned char *)((char *)INSTALL_ADDR + 0xa0000000);
        
        while (len--) {
            loc[len] = wiiurl532_text_bin[len];
        }

        /* server IP address */
        ((unsigned int *)loc)[0] = ip.full; //PC_IP;

        DCFlushRange(loc, sizeof(wiiurl532_text_bin));

        struct magic_t {
            void *real;
            void *replacement;
            void *call;
        } *magic = (struct magic_t *)wiiurl532_magic_bin;
        len = sizeof(wiiurl532_magic_bin) / sizeof(struct magic_t);
        
        int *space = (int *)(loc + sizeof(wiiurl532_text_bin));
